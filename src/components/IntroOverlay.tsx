...
export default function IntroOverlay({ onDone }: { onDone: () => void }) {
  ...
  // (3) the 4 screens emerge as the box collapses away
  ...
  const panel: Variants = {
    hidden: { opacity: 0, y: 34, scale: 0.9, filter: 'blur(6px)' },
      ...
      <div className="intro-stage">
        ...
        <motion.div className="intro-center" variants={drift} initial="hidden" animate="show">
          ...
          <motion.div className="intro-grid2" variants={grid} initial="hidden" animate="show">
            ...
            <motion.div className="intro-panel" variants={panel}>
              <div className="ip-orb" />
              <div className="scr-name">Acme Corp</div>
              <div className="ip-score">92</div>
              <div className="ip-bar"><i style={{ width: '92%' }} /></div>
              <div className="ip-state">Healthy</div>
            </motion.div>
            <motion.div className="intro-panel" variants={panel}>
              <div className="scr-chans">
                {CHANS.map((s, i) => <span className="scr-chan" key={i}><img src={s} alt="" /></span>)}
              </div>
              <div className="scr-title">Every channel,<br />one brain</div>
            </motion.div>
            <motion.div className="intro-panel" variants={panel}>
              <div className="ip-cap">Founder brief</div>
              <div className="ip-brief">
                <div className="ip-bl"><span className="d" style={{ background: 'var(--err)' }} /><div><b>Acme</b> quiet 9d · $144k</div></div>
                <div className="ip-bl"><span className="d" style={{ background: 'var(--warn)' }} /><div>3 promises due today</div></div>
                <div className="ip-bl"><span className="d" style={{ background: 'var(--ok)' }} /><div>Northwind heating up</div></div>
              </div>
            </motion.div>
          </motion.div>
        </motion.div>
